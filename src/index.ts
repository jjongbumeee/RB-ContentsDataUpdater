...
class OutputSample {
  ...
  constructor(id, date, date_gmt, type, link, title, content,
    featured_media, categories, tags, featured_image_src) {
    this.id = id;
    this.date = date;
...
const sampleFunction = (tag: any[]) => {
  ...
  for (let i in tag) {
    ...
    let tmp = new OutputSample(now.category0, now.CreatedAt, now.UpdatedAt,
      site, now.fullUrl,{ rendered: site + " " + now.searchKeyword }, content,
      0, category, [], now.thumbnailUrl);
